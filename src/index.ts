...
export default (
  ...
  async function handleHtmlTransformation(
    html: string
  ...
  function printStats(
    ...
    const cliMsgValue = chalk`(${originalSize.toString()} kB → {${
  ...
  function handleResizeWidth(
    ...
    if (_width && isNaN(_width))
      console.error(
        `Parameter width with value ${width} is not parsable to integer.`
      );
    if (_height && isNaN(_height))
      console.error(
        `Parameter width with value ${height} is not parsable to integer.`
      );
    sharpImage.resize({
      width: _width,
      height: _height,
  ...
  async function handleConversion(
    ...
    const parsedQuality =
      quality && typeof quality === "string" ? parseInt(quality) : null;
  ...
  function parseRGBColor(color: sharp.Color) {
    ...
    if (typeof color === "object")
      color = `${color.r.toString(16)}${color.g.toString(16)}${color.b.toString(
        16
      )}`;
    color = color.replace("#", "");
